...
function draw(){
    ...
    if (flock) {
        ...
        for (var i = 0; i < f.length; i++) { 
            ...
            //ellipse(f[i]['position'][0], f[i]['position'][1], 10, 10)
            show(particle=f[i]);
...
function show(particle){
    ...
    velocity = createVector(parseFloat(particle.velocity[0]), parseFloat(particle.velocity[1]));
    position = createVector(parseFloat(particle.position[0]), parseFloat(particle.position[1]));
    translate(position);
    ...
    line(position.x, position.y, 
        position.x + (velocity.x*5), position.y + (velocity.y*5));

    if (show_perc){
        // circle to display perception radius
        stroke(0, 255, 0);
        noFill();
        ellipse(particle.position[0], particle.position[1], 
            (particle.free_space)*2, (particle.free_space)*2);
        arc(particle.position[0], particle.position[1], 
